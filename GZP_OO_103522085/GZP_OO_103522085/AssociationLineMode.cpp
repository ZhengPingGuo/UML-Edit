...
void AssociationLineMode::OnLButtonDown(CPoint point, MyDocument* doc)
{
...
void AssociationLineMode::OnMouseMove(CPoint, MyDocument*)
{

}

void AssociationLineMode::OnLButtonUp(CPoint point, MyDocument* doc)
{
	if (is_mouse_down == 1)
	{
		is_mouse_down = 0;
		...
		if (shape_ptr != NULL)
		{
			if (use_for_Obj != shape_ptr && !(use_for_Obj->isGroupClass()) && !(shape_ptr->isGroupClass()))
			{
				...
				AssociationLine *a_line = new AssociationLine(use_for_Obj->getMyPort(temp_num.x), shape_ptr->getMyPort(temp_num.y));
				doc->addObj(a_line);
			}
		}
	}
}
CPoint AssociationLineMode::determinePosition(CPoint *c1, CPoint *c2)
{
	CPoint p;
